...
async function getJWT() {
  ...
  cachedJWT = {
    ...
    deviceId: clientID,
    country: data.session?.countryCode,
  };
  cachedJWTTime = now;
...
app.get("/health", async (req, res) => {
  try {
    ...
    res.json({
      ...
      channel: ch?.name,
      slug: ch?.slug,
      isUS: ch?.slug ? !ch.slug.includes("-de") && !ch.slug.includes("-gb") && !ch.slug.includes("-fr") : false,
    });
  } catch (e) {
...
// ============================================
setInterval(() => {
  fetch(`${API}/v2/channels?channelType=live&limit=1`, { headers: plutoHeaders() })
    .then(() => console.log("💓"))
    .catch(() => {});
}, 10 * 60 * 1000);
